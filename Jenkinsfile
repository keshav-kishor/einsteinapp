   ...
   stage('Installing dependency packages') {
	...
	withCredentials([file(credentialsId: JWT_KEY_CRED_ID, variable: 'jwt_key_file')]) {
		stage('Apex Test Execution') {
			...
			if (isUnix()) {
                rc = sh returnStatus: true, script: "npm run sfdx -- force:auth:jwt:grant --clientid ${CONNECTED_APP_CONSUMER_KEY} --username ${HUB_ORG} --jwtkeyfile ${jwt_key_file} --setdefaultdevhubusername --instanceurl ${SFDC_HOST}"
            }else{
        ...
        stage('Deploy Source Code') {
            ...
            if (isUnix()) {
                rc = sh returnStatus: true, script: "npm run sfdx -- force:auth:jwt:grant --clientid ${CONNECTED_APP_CONSUMER_KEY} --username ${HUB_ORG} --jwtkeyfile ${jwt_key_file} --setdefaultdevhubusername --instanceurl ${SFDC_HOST}"
                 ...
                 rc = bat returnStatus: true, script: "npm run sfdx -- force:auth:jwt:grant --clientid ${CONNECTED_APP_CONSUMER_KEY} --username ${HUB_ORG} --jwtkeyfile \"${jwt_key_file}\" --setdefaultdevhubusername --setalias DevHubOrg --instanceurl ${SFDC_HOST}"
            }
            if (rc != 0) { error 'hub org authorization failed' }
			println rc
